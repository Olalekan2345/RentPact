import "server-only";
import { recordActivityEvent } from "@/lib/activityEventServer";

/**
 * Self-heals the activity feed from the chain. Releases are mirrored into
 * activity_events by a fire-and-forget write at release time (leaseData.ts);
 * when that write drops, the release is on-chain but missing from the feed, so
 * it never shows in transaction history or the lease record. This reconstructs
 * any missing releases from the current contract's TrancheReleased logs and
 * upserts them — so the feed converges on the chain instead of depending on one
...
/** Top up the feed's release rows for the given leases from the current contract. */
export async function reconcileReleasesFromChain(leaseIds: string[]): Promise<void> {
  if (!ESCROW) return;
  const now = Date.now();
  ...
  try {
    ...
    for (let page = 0; page < 20; page++) {
      ...
      for (const log of body.items ?? []) {
        ...
        if (topic0.toLowerCase() !== TRANCHE_RELEASED_TOPIC) continue;
        const leaseId = BigInt(topic1).toString();
        if (!wanted.has(leaseId)) continue;
        upserts.push(
          recordActivityEvent({
            id: `${log.transaction_hash}-release`,
            leaseId,
            type: "release",
            timestamp: Date.parse(log.block_timestamp),
            amount: Number(dataWord(log.data, 1)) / 10 ** USDC_DECIMALS,
