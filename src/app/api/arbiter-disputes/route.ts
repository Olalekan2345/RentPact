...
/**
 * Every lease that has ever had a (non-caution) dispute raised, for the arbiter
 * view. The arbiter isn't a party to any lease, so it can't discover disputes
 * from its own lease lists — this reads the disputed lease ids from the Arcscan
 * indexer (fast; the RPC full-log scan is far too slow). The client then reads
 * each lease's live state to keep only the ones still active.
 *
 ...
 * — the dispute data is public on-chain and the actual ruling is enforced
 * on-chain (only the arbiter wallet can resolve).
 */
      ...
      for (const log of body.items ?? []) {
        ...
        try {
          ...
          if (decoded.eventName === "DisputeRaised") {
            const { leaseId, reason } = decoded.args as { leaseId: bigint; reason: string };
            if (reason !== CAUTION_CLAIM_DISPUTE_REASON) ids.add(leaseId.toString());
          }
        } catch {
